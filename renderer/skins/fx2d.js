...
export function makeFx(getGeom) {
  ...
  function drawLayers(ctx, below) {
    // 贴地的光环画在脚下，脚就在窗口最底下——半圈会被窗口边切掉，看着像画歪了。
    // 所以贴地那几种往上顶一点，保证整圈都在画面里。
    const floor = (getGeom().feet || {}).y;
    const lift = (l, ry) => (floor == null ? l.y : Math.min(l.y, floor + 4 - ry));
    for (const l of layers) {
      ...
      ctx.save();
      if (l.type === "ring") { const r = l.r0 + (l.r1 - l.r0) * (1 - Math.pow(1 - k, 2)); ctx.globalAlpha = fadeOut * 0.9; ctx.strokeStyle = l.color; ctx.lineWidth = l.width * (1 - k * 0.5); ctx.beginPath(); ctx.ellipse(l.x, lift(l, r * 0.32), r, r * 0.32, 0, 0, TAU); ctx.stroke(); }
      ...
      else if (l.type === "magic") { const a = Math.min(1, l.t * 3, fadeOut * 2.5); ctx.globalAlpha = a; ctx.strokeStyle = l.color; ctx.lineWidth = 2; const r = l.r, ry = r * 0.32; ctx.translate(l.x, lift(l, ry)); for (const [rr, dir] of [[r, 1], [r * 0.72, -1]]) { ctx.beginPath(); ctx.ellipse(0, 0, rr, rr * 0.32, 0, 0, TAU); ctx.stroke(); for (let i = 0; i < 12; i++) { const ang = i / 12 * TAU + l.t * 1.4 * dir; ctx.beginPath(); ctx.arc(Math.cos(ang) * rr, Math.sin(ang) * rr * 0.32, i % 3 === 0 ? 2.6 : 1.4, 0, TAU); ctx.fillStyle = l.color; ctx.fill(); } } ctx.beginPath(); for (let i = 0; i < 3; i++) { const ang = i / 3 * TAU + l.t * 0.9; const px = Math.cos(ang) * r * 0.86, py = Math.sin(ang) * ry * 0.86; i ? ctx.lineTo(px, py) : ctx.moveTo(px, py); } ctx.closePath(); ctx.stroke(); }
