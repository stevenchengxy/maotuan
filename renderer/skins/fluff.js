...
export function makeFluff(canvas, opts = {}) {
  ...
  function render(g) {
    ...
    ctx.rotate(S.tilt + S.look.x * 0.035);
    ctx.scale(sx, sy);
    ctx.translate(-cx, -(cy + R * 0.95));
  ...
  return {
    ...
    land: (k = 1) => { S.squash = 1 + 0.22 * k; S.hop = 0; const g = geometry(); spawn("dot", 5, g.cx, g.cy + g.R); },
    spin: () => { S.beh = { type: "wiggle", t: 0, dur: 0.9 }; const g = geometry(); spawn("heart", 2, g.cx, g.cy - g.R); },
