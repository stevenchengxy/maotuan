...
// names.js / tapTables.js 里还没写 term 时也能跑：这里留一份名字和口气兜底
const DEF = { name: "小终", style: "短句直给，先给结论，事情办完就一句「已执行」；冷静但不冷淡。", ...withTaps(SKIN_DEFAULTS.term, "term") };
...
export const makeTerm = makeVectorSkin({
  ...DEF, character: DEF,
  rK: 0.21, iconK: 0.3, baseYK: 0.62, headK: 0.55, hit: [1.15, 1.12, 0.06],
  ...
  tick(S, dt, g, { spawn }) {
    ...
    if (E.scroll > 1e6) E.scroll -= 1e6;   // 开着一整天也不让它涨到丢精度
    ...
    if (S.mood === "happy" && Math.random() < dt * 1.1) spawn("dot", 1, g.cx + (Math.random() - 0.5) * g.R * 1.2, g.cy - g.R * 0.6);
  },
  ...
  draw(ctx, g, S, face, X) {
    ...
    // 影子：留在地上，蹦起来就小一点淡一点
    const lift = Math.max(0, Math.min(1, (g.baseY - cy) / (R * 0.5)));
    ctx.save(); ctx.globalAlpha = 0.22 * (1 - lift * 0.45);
    ctx.fillStyle = "rgb(25,35,30)"; ctx.beginPath();
    ctx.ellipse(cx, g.baseY + R * 1.04, R * 0.92 * (1 - lift * 0.18), R * 0.12 * (1 - lift * 0.25), 0, 0, TAU); ctx.fill(); ctx.restore();
    ...
    // 顶上那行暗暗的"路径"
    ctx.save(); ctx.globalAlpha = 0.45 * dim; ctx.fillStyle = DIMG;
    for (const [ox, ow] of [[0.08, 0.16], [0.27, 0.10], [0.40, 0.22]]) { ctx.beginPath(); ctx.roundRect(sx + sw * ox, sy + sh * 0.12, sw * ow, R * 0.045, R * 0.022); ctx.fill(); }
    ctx.restore();

    // 眼睛：两个方块光点，眨眼时压扁成一条，开心时变成两个尖角
    const lx = S.look.x * R * 0.09, ly = S.look.y * R * 0.05;
    const ey = sy + sh * 0.31 + ly, ew = R * 0.17, eh = sh * 0.20;
    const cl = Math.max(closed, yawn > 0.3 ? 1 : 0);
    ctx.save(); ctx.globalAlpha = Math.max(0.6, dim);
    ctx.shadowColor = GLOW; ctx.shadowBlur = R * (0.05 + S.pet.amt * 0.05 + flash * 0.06) * dim;
    ...
    if (S.talking || S.mouthLevel > 0.05) {
      ...
      for (let i = 0; i < 5; i++) {
        const bhh = sh * (0.02 + o * 0.11 * (0.5 + 0.5 * Math.sin(t * 22 + i * 1.7)));
        ctx.beginPath(); ctx.roundRect(cx - R * 0.345 + i * R * 0.15 + lx, my - bhh, R * 0.09, bhh + R * 0.02, R * 0.02); ctx.fill();
      }
    } else if (S.mood === "thinking") {
      const w = R * 0.92, k = (t * 0.42) % 1;
      ctx.strokeStyle = DIMG; ctx.lineWidth = Math.max(1, R * 0.022);
      ctx.beginPath(); ctx.roundRect(cx - w / 2, my - sh * 0.05, w, sh * 0.10, sh * 0.05); ctx.stroke();
      ctx.fillStyle = GREEN; ctx.beginPath(); ctx.roundRect(cx - w / 2 + R * 0.02, my - sh * 0.032, (w - R * 0.04) * k, sh * 0.064, sh * 0.032); ctx.fill();
      ...
      ctx.beginPath(); ctx.moveTo(cx - R * 0.30 + lx, my - R * 0.07); ctx.lineTo(cx - R * 0.21 + lx, my); ctx.lineTo(cx - R * 0.30 + lx, my + R * 0.07); ctx.stroke();
      ctx.fillStyle = happy ? GREEN : DIMG;
      ctx.beginPath(); ctx.roundRect(cx - R * 0.13 + lx, my - R * 0.025 + (happy ? Math.sin(t * 6) * R * 0.02 : 0), R * (happy ? 0.36 : 0.26), R * 0.05, R * 0.025); ctx.fill();
    }
    ctx.restore();

    // 两三行假代码 + 会闪的方块光标（整块裁掉，滚动时从下面顶上来）
    const caY = sy + sh * 0.72, caH = sh * 0.26, x0 = sx + sw * 0.08, cw = sw * 0.84, rowH = caH / 2;
    ...
    // 光标：说话时常亮，想事情闪得快，困了慢慢闪（相位固定，图标里那一帧也一定看得见）
    const hz = S.talking ? 0 : S.mood === "thinking" ? 2.6 : S.mood === "sleepy" ? 0.5 : 1.2;
    if (hz === 0 || (t * hz) % 1 < 0.6) {
      ctx.save(); ctx.globalAlpha = Math.max(0.55, dim); ctx.shadowColor = GLOW; ctx.shadowBlur = R * 0.05 * dim;
      ctx.fillStyle = GREEN;
    ...
    if (pop > 0) {
      const k = 1 - pop;
      ctx.save(); ctx.globalAlpha = pop * dim; ctx.fillStyle = LIT;
      for (let i = 0; i < 4; i++) {
    ...
    // 扫描线：几条半透明横线，慢慢往下走
    ctx.save(); ctx.globalAlpha = 0.07 * dim; ctx.fillStyle = "#CFFFE4";
    const band = sh / 5, off = (t * 11) % band, lw = Math.max(1, R * 0.022);
    for (let i = -1; i < 5; i++) ctx.fillRect(sx, sy + off + i * band, sw, lw);
    ctx.restore();

    // 开心时整屏闪一下绿光
    if (flash > 0) { ctx.save(); ctx.globalAlpha = 0.18 * flash; ctx.fillStyle = GREEN; ctx.fillRect(sx, sy, sw, sh); ctx.restore(); }
    ...
    ctx.save(); ctx.shadowColor = GLOW; ctx.shadowBlur = R * 0.04;
    ...
    ctx.save(); ctx.globalAlpha = 0.35; ctx.fillStyle = "#7C848E";
