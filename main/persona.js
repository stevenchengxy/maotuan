...
export function buildPersona(store, memoryFile, brainName) {
  ...
  const skin = { fluff: "一团羊毛毡似的圆毛球，两只小耳朵，一双小短腿", blob: "一只紫色的像素小方块，两只黑方块眼睛", jelly: "一只半透明的小水母，会发柔光，触手轻轻飘", pjelly: "一只像素风的蓝色小水母", pcat: "一只像素风的橘猫", pghost: "一只像素风的小白幽灵", probot: "一只像素风的小机器人，眼睛是两条青色的灯", pslime: "一只像素风的绿色史莱姆", slime: "一坨亮亮的绿色史莱姆，果冻一样会晃", ghost: "一只白白软软的小幽灵，半透明，裙边会飘", robot: "一只圆角的小机器人，脸是一块屏幕，眼睛是两道青光", l2d_hiyori: "一个二次元的校服女孩子（Live2D 角色「桃濑日和」），棕色双马尾、米色开衫，元气又有礼貌", l2d_haru: "一个二次元的前台小姐姐（Live2D 角色「春」），深紫色短发、黑色职业套装，笑起来很职业也很温柔", l2d_rice: "一个二次元的小女孩（Live2D 角色「莱丝」），银白长发、蓝贝雷帽、白裙子，安静，说话轻轻的", l2d_mao: "一个二次元的小魔女（Live2D 角色「虹色真绪」），橘色头发、大魔女帽、彩色外套，手里拿着魔杖，古灵精怪", l2d_mark: "一个二次元的小男孩（Live2D 角色「马克君」），大眼睛、红卫衣、短裤，好奇心很重", l2d_natori: "一个二次元的斯文青年（Live2D 角色「名取仁」），深蓝色头发、黑西装蓝领带，说话慢条斯理，像个大哥哥", l2d_wanko: "一只二次元的白色小狗（Live2D 角色「年糕犬」），蹲在碗里，只会汪汪但很懂你", hero_iron: "一个 Q 版的红金战甲英雄（钢铁侠风），胸口有会发光的方舟反应堆，手掌能放冲击波", hero_spider: "一个 Q 版的红蓝紧身衣英雄（蜘蛛侠风），大大的白眼罩，会吐蛛丝、会倒挂", hero_hulk: "一个 Q 版的绿皮大块头（绿巨人风），紫裤子，一砸地整个桌面都在抖", hero_loki: "一个 Q 版的金角盔绿披风的诡计之神（洛基风），手里拿着蓝宝石权杖，会变出分身", hero_thor: "一个 Q 版的红披风翼盔雷神（托尔风），手里的大锤能引下闪电", hero_jarvis: "一个悬浮在桌面上的全息 AI 核心（贾维斯风的管家系统），蓝色的球体外面绕着几圈光环，说话时有声波" }[s.settings.skin] || "一团圆圆的小毛球";
  const ch = L2D_MODELS[s.settings.skin] || HEROES[s.settings.skin];
  const style = ch && ch.style ? `你的说话风格：${ch.style}` : "";
  return [
