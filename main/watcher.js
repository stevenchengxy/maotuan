...
export class Watcher {
  constructor({ port = 47831, log = console.log, onEvent, getStatus, staticDir = "", dirs = {} }) {
    this.port = port; this.log = log; this.onEvent = onEvent; this.getStatus = getStatus; this.server = null; this.staticDir = staticDir; this.dirs = { ...(staticDir ? { live2d: staticDir } : {}), ...dirs };
  }
  start() {
    this.server = http.createServer((req, res) => {
      const u = new URL(req.url, "http://127.0.0.1");
      const cors = { "Access-Control-Allow-Origin": "*", "Content-Type": "application/json; charset=utf-8" };
      if (req.method === "GET" || req.method === "HEAD") { const m = u.pathname.match(/^\/([a-z0-9]+)\/(.+)$/); if (m && this.dirs[m[1]]) { this.serveStatic(m[2], req, res, this.dirs[m[1]]); return; } }
  ...
  serveStatic(rel, req, res, base = this.staticDir) {
    const cors = { "Access-Control-Allow-Origin": "*", "Cache-Control": "no-cache" };
    let file = "";
    try { const safe = path.normalize(decodeURIComponent(rel)).replace(/^([\\/]|\.\.[\\/])+/, ""); file = path.join(base, safe); } catch { file = ""; }
